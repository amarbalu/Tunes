...
app.post("/upload", uploadFile.single("file"), (req, res) => {
  const gridFSBucket = new mongoose.mongo.GridFSBucket(connect.db, {
    bucketName: "uploads",
  });
  try {
...
app.delete("/trashit/:trackID", async (req, res) => {
  try {
    ...
    gridFSBucket.delete(req.params.trackID, function (error) {
      if (!error) {
        res.status(201).json({
          success: true,
...
app.get("/files/:trackID", async (req, res) => {
  ...
  try {
    ...
    await gridFSBucket.find({ _id: trackID }).toArray((err, files) => {
      if (!files || files.length === 0) {
      ...
      const rangeRequest = readRangeHeader(
        req.headers["range"],
        files[0].length
      );
      ...
      if (rangeRequest === null) {
        ...
        sendResponse(
          ...
          200,
          responseHeaders,
          gridFSBucket.openDownloadStream(trackID)
        );
        return null;
      }
      ...
      responseHeaders["Content-Range"] =
        "bytes " + start + "-" + end + "/" + files[0].length;
